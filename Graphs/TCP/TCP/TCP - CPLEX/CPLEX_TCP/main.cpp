...
void RunInstance(std::string & InstanceDir, int L, int R, int primal_bound, long memory, long time) {
    ...
    IloIntVar var_l(env, 0, IloInfinity);
    IloIntVar var_r(env, 0, IloInfinity);
    ...
    if(link_router)
    {
        ...
        model.add(IloSum(l) == var_l);
        model.add(IloSum(r) == var_r);
        model.add(var_l <= L+R);
        model.add(var_r <= R);
    }
    ...
    // cout << "Soma de L = " << cplex.getValue(var_l)<< endl;
    // cout << "Soma de R = " << cplex.getValue(var_r)<< endl;
}
//
TEdges LoadEdges(TEdges & InstData)
{
